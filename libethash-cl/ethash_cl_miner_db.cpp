...
void ethash_cl_miner::search(uint8_t const* header, uint64_t target, search_hook& hook, bool _ethStratum, uint64_t _startN)
{
	try
	{
		...
		bool run_flag[c_bufferCount] = {true, false};
		...
		do
		{
			...
			if (run_flag[buf])
			{
				...
				hook.searched(start_nonce, m_globalWorkSize);
			}
			// check if search should continue
			run_flag[buf] = ! hook.searched(start_nonce, 0);
		} while (run_flag[0] || run_flag[1]);
